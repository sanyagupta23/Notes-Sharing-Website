...
def admin_home(request):
    ...
    d = {'pendnts':pendnts , 'accnts':accnts , 'rjctnts':rjctnts , 'allnts':allnts }
...
def user_home(request):
    if not request.user.is_authenticated:                                                        # redirect ke andar userlogin function ko define karte time (name="") ke andar jo value daali thi na "login" wo daali hai .(login.html file ka reference ni hai ye)
        return redirect('login')
...
def upload_notes(request):
    ...
    if request.method == 'POST':
        ...
        try:
            Notes.objects.create(user=crnt_u, uploadingdate=date.today(), branch=b, subject=s, notesfile=n,filetype=f,description=d,status='pending')
            error = "no"
        except:
            error = "yes"
    d = {'error': error}
